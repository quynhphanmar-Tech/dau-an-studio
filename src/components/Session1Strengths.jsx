...
import { Mic, MicOff, ArrowRight, Paperclip, Lightbulb, ChevronRight } from 'lucide-react';
...
const QUESTIONS = [
  ...
  },
  {
    ...
    subtitle: 'Kể lại thành tựu bạn tự hào nhất — bằng giọng nói hoặc gõ phím.',
    type: 'voice-or-text',
    field: 'biggestWin',
...
export default function Session1Strengths({ profile, updateProfile, onNext }) {
  ...
  const handleToggleRecording = () => {
    if (isRecording) {
      setIsRecording(false);
      updateProfile({ [q.field]: `[Đã ghi âm ${formatTime(recordingTime)}] ${inputValue}` });
    } else {
      setIsRecording(true);
      setRecordingTime(0);
    }
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-12 min-h-[calc(100vh-80px)] flex flex-col justify-between animate-fade-in-up">
      ...
      <div className="flex-1 flex flex-col justify-center" key={step}>
        ...
        {q.subtitle && (
          <p className="text-base text-ink/50 leading-relaxed mb-8 max-w-md">
            {q.subtitle}
          </p>
        )}
        ...
        {q.type === 'intro' && (
          <div className="mt-8 space-y-4">
            ...
            <button
              onClick={handleNext}
              className="mt-6 inline-flex items-center gap-2 bg-ink text-cream px-7 py-3.5 rounded-full text-sm font-semibold hover:bg-ink/90 transition-all active:scale-95"
            >
              Bắt đầu
        ...
        {/* Voice-or-Text Input Screen (Key Screen from ExpertPrint Design) */}
        {q.type === 'voice-or-text' && (
          <div className="mt-4 space-y-6">
            {/* Voice Recording Area */}
            <div className="bg-white rounded-2xl border border-silver/80 p-6 space-y-5">
              ...
              {/* Timer */}
              {isRecording && (
                <p className="text-center text-sm font-mono text-ink/60">
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <span className="text-xs font-medium text-ink/50 uppercase tracking-wider">
                  {isRecording ? 'Dừng ghi' : 'Kể lại bằng giọng nói'}
                </span>
              </div>
            </div>
          ...
          <div className="mt-6 space-y-6">
            {!summaryReady ? (
              ...
              <div className="space-y-6 animate-fade-in-up">
                ...
                {/* Readiness indicator (NOT a score) */}
